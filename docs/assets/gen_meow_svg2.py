...
def make_mode_field(w, h):
    ...
    # Re-zero outside an eroded mask to prevent contour leakage
    from scipy.ndimage import binary_erosion

    eroded = binary_erosion(mask, iterations=int(SMOOTH_SIGMA * 1.5))
    field[~eroded] = 0
...
def main():
    ...
    # Contour levels: start well inside the boundary to avoid spillover
    levels = np.linspace(0.08, 0.95, N_LEVELS)

    print("Extracting contours...")
    contour_lines = extract_contour_lines(field, levels)

    # Extract the cat silhouette boundary for the cross-section underlay
    boundary_contours = extract_contour_lines(mask.astype(float), [0.5])
    cat_boundary_d = ""
    if boundary_contours and boundary_contours[0]:
        largest_boundary = max(boundary_contours[0], key=len)
        cat_boundary_d = polygon_to_svg_path(largest_boundary)
    ...
    # Light blue cat-shaped cross-section underlay with dark outline
    if cat_boundary_d:
        lines.append(
            f'  <path d="{cat_boundary_d}" fill="#d4e6f6" stroke="#1a1a3a" stroke-width="2.0"/>'
        )
        # Clip contours to cat boundary
        lines.append("  <defs>")
        lines.append(
            f'    <clipPath id="cat-clip"><path d="{cat_boundary_d}"/></clipPath>'
        )
        lines.append("  </defs>")

    # Contour lines: stroke only, no fill, clipped to cat boundary
    lines.append('  <g clip-path="url(#cat-clip)">')
    for i, polygons in enumerate(contour_lines):
        ...
        # Thicker lines for outer contours, thinner for inner
        sw = 2.5 - i * 0.1
        sw = max(sw, 1.2)
        if path_d:
            lines.append(
